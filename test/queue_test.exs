defmodule QueueTest do
  ...
  describe "to_list/1" do
    ...
    test "reflects elements already popped" do
      q = Queue.new()
      Queue.push_back_n(q, [1, 2, 3, 4, 5])

      assert 1 == Queue.pop_front(q)
      assert [2, 3, 4, 5] == Queue.to_list(q)

      assert [2, 3] == Queue.pop_front_n(q, 2)
    ...
    test "is empty again after the queue is drained" do
      ...
      assert [1, 2, 3] == Queue.pop_front_n(q, 3)
    ...
    test "spans slab boundaries after a partial pop" do
      ...
      popped = 4096 + 100
      assert Enum.take(expected, popped) == Queue.pop_front_n(q, popped)
    ...
    test "agrees with len/1 as the queue is consumed" do
      q = Queue.new()
      Queue.push_back_n(q, Enum.to_list(1..100))

      Enum.each(1..10, fn _ ->
        Queue.pop_front_n(q, 7)
  ...
  describe "count/1" do
    ...
    test "does not go negative when over-popping" do
      ...
      assert [1, 2, 3] == Queue.pop_front_n(q, 1000)
      assert 0 == Queue.count(q)
    ...
    test "spans slab boundaries" do
      ...
      q = Queue.new()
      Queue.push_back_n(q, Enum.to_list(1..n))

      assert n == Queue.count(q)

      popped = 4096 + 100
      Queue.pop_front_n(q, popped)
    ...
    test "tracks length through interleaved pushes and pops" do
      ...
      Enum.reduce(1..50, 0, fn i, expected ->
        Queue.push_back_n(q, Enum.to_list(1..i))
        popped = length(Queue.pop_front_n(q, div(i, 2)))
  ...
  describe "push_back/2" do
    ...
    test "interleaves correctly with push_back_n/2" do
      q = Queue.new()

      Queue.push_back(q, 1)
      Queue.push_back_n(q, [2, 3])
      Queue.push_back(q, 4)
  ...
  describe "push_back_n/2" do
    test "returns the same queue it was given" do
      q = Queue.new()

      assert q == Queue.push_back_n(q, [:a])
    end

    test "mutates in place, so the return value and the original alias" do
      q = Queue.new()
      q2 = Queue.push_back_n(q, [:a, :b])

      Queue.push_back_n(q2, [:c])
    ...
    test "can be piped" do
      q =
        Queue.new()
        |> Queue.push_back_n([1, 2])
        |> Queue.push_back_n([3, 4])
    ...
    test "appends in list order, to the back" do
      ...
      Queue.push_back_n(q, [:a, :b])
      Queue.push_back_n(q, [:c, :d])
    ...
    test "keeps duplicates as separate elements" do
      ...
      Queue.push_back_n(q, [:dup, :dup, :dup])
    ...
    test "keeps its own copy after the pushing process dies" do
      ...
      pid =
        spawn(fn ->
          Queue.push_back_n(q, [{:from_child, :binary.copy("y", 100_000)}])
          send(parent, :pushed)
        end)
    ...
    test "fills a slab exactly, then rolls over" do
      ...
      Queue.push_back_n(q, Enum.to_list(1..4096))
      assert 4096 == Queue.count(q)

      Queue.push_back_n(q, [:overflow])
    ...
    test "rejects a non-list without touching the queue" do
      ...
      assert_raise FunctionClauseError, fn -> Queue.push_back_n(q, not_a_list) end
    ...
    #   assert_raise ErlangError, fn -> Queue.push_back_n(q, [:b | :c]) end
    ...
    test "each concurrent batch lands contiguously" do
      # push_back_n holds the queue lock for the whole batch, so batches
      ...
      tasks =
        for p <- 1..10 do
          Task.async(fn ->
            Queue.push_back_n(q, for(i <- 1..batch_size, do: {p, i}))
          end)
        end
  ...
  describe "pop_front/1" do
    ...
    test "interleaves correctly with pop_front_n/2" do
      q = Queue.new()
      Queue.push_back_n(q, Enum.to_list(1..6))

      assert 1 == Queue.pop_front(q)
      assert [2, 3] == Queue.pop_front_n(q, 2)
      assert 4 == Queue.pop_front(q)
      assert [5, 6] == Queue.pop_front_n(q, 10)
      assert nil == Queue.pop_front(q)
    end
  ...
  describe "pop_front_n/2" do
    test "an empty queue returns an empty list" do
      assert [] == Queue.pop_front_n(Queue.new(), 5)
    end
    ...
    test "asking for one element behaves like pop_front/1" do
      ...
      assert [:a] == Queue.pop_front_n(q, 1)
      assert :b == Queue.pop_front(q)
    end
    ...
    test "asking for more than is available returns what there is" do
      ...
      assert [] == Queue.pop_front_n(q, 1000)
    end
    ...
    test "preserves arbitrary terms" do
      ...
      assert terms == Queue.pop_front_n(q, length(terms))
    end
    ...
    test "preserves large and deeply nested terms" do
      ...
      assert terms == Queue.pop_front_n(q, 3)
    end
    ...
    test "a single call can span many slabs" do
      ...
      assert expected == Queue.pop_front_n(q, length(expected))
      assert 0 == Queue.count(q)
      assert [] == Queue.to_list(q)
    end
    ...
    test "repeated partial pops drain a multi-slab queue in order" do
      ...
      drained =
        Stream.repeatedly(fn -> Queue.pop_front_n(q, chunk) end)
        |> Enum.take_while(&(&1 != []))
        |> List.flatten()
    ...
    test "a pop that lands exactly on a slab boundary leaves the rest intact" do
      q = Queue.new()
      Queue.push_back_n(q, Enum.to_list(1..(4096 + 5)))

      assert Enum.to_list(1..4096) == Queue.pop_front_n(q, 4096)
      assert 5 == Queue.count(q)
      assert Enum.to_list(4097..4101) == Queue.to_list(q)
      assert Enum.to_list(4097..4101) == Queue.pop_front_n(q, 5)
    end
    ...
    test "works again after the queue was drained" do
      ...
      assert [1, 2, 3] == Queue.pop_front_n(q, 3)
      assert [] == Queue.pop_front_n(q, 3)

      Queue.push_back_n(q, [4, 5])
      assert [4, 5] == Queue.pop_front_n(q, 3)
    end
    ...
    test "sees elements pushed between pops" do
      ...
      assert [1] == Queue.pop_front_n(q, 1)

      Queue.push_back_n(q, [3, 4])
      assert [2, 3] == Queue.pop_front_n(q, 2)

      Queue.push_back(q, 5)
      assert [4, 5] == Queue.pop_front_n(q, 5)
    end
    ...
    test "rejects a negative count without touching the queue" do
      ...
      assert_raise FunctionClauseError, fn -> Queue.pop_front_n(q, negative) end
    ...
    test "rejects a non-integer count without touching the queue" do
      ...
      assert_raise FunctionClauseError, fn -> Queue.pop_front_n(q, not_an_integer) end
    ...
    test "each concurrent batch is a contiguous run" do
      # pop_front_n holds the queue lock for the whole batch, so concurrent
  ...
  describe "Enumerable" do
    ...
    test "agrees with Queue.count/1" do
      q = Queue.new()
      Queue.push_back_n(q, Enum.to_list(1..10))
    ...
    test "nil elements are counted like any other term" do
      q = Queue.new()
      Queue.push_back_n(q, [nil, nil, nil])
